import { RefreshedTokens, SuccessLogin } from '../../src/models/authentication'
import { ErrorResponse } from '../../src/models/response'
import { HTTP_STATUS } from '../../src/util/endpoint-util'
import { login, ACCOUNTS, getTokens, refreshTokens } from './helper'
...
describe('Refresh tokens tests', () => {
  it('Refresh valid access token', async () => {
    ...
    // tokens still valid so they are the same
    expect(firstTokens.accessToken).toBe(accessToken)
    expect(firstTokens.refreshToken).toBe(refreshToken)
  })
